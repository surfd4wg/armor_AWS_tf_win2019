...
data "aws_ami" "amazon_windows_2019" {
  ...
  filter {
    name   = "name"
    values = ["*Windows_Server-2019-English-Full-Base-2021.03.10*"]
  }
...
# Ec2 user data, WinRM and PowerShell Provision Functions
   user_data = <<EOF
	...
	#IIS
		...
		Invoke-WebRequest https://armorscripts.s3.amazonaws.com/WINscripts/WIN2019.jpg -outfile c:\inetpub\wwwroot\WIN2019.jpg
		Invoke-WebRequest https://armorscripts.s3.amazonaws.com/WINscripts/iisstart2019.htm -outfile c:\inetpub\wwwroot\iisstart.htm
	...
	tags = {
		...
		Name = "WindowsServer2019-Terraform-${count.index + 1}"
	}
